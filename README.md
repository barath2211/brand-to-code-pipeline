...
> **About this repo:** An open-source reference implementation of a client onboarding automation I designed, built and rolled out with a colleague at an enterprise SaaS company. It reproduces the approach, not the original code. All brands, content and data here are fictional.
>
> **Read the [case study](docs/CASE_STUDY.md):** delivery went from 3 to 6 weeks down to between 2 hours and 2 days, across 10 to 30 clients so far.
...
![Four fictional brands generated by the pipeline](docs/img/brands.png)

*Same components, four brands. Helix Bio (2nd) failed QA and was auto-repaired, and its broken logo falls back to a text wordmark. Nordlys Energy (3rd) was built from guideline text and Kestrel Logistics (4th) from a PDF brand book.*
...
```mermaid
flowchart LR
    G[Brand guidelines<br>PDF or text] -->|LLM extract| C[Brand config JSON]
...
1. **Extract** (optional LLM step): a PDF brand book or free-text guidelines become a structured config. The model only reads; the schema decides if the result is usable.
...
Build straight from a brand book (uses the LLM extraction step):

```bash
python src/cli.py build data/brands/kestrel-logistics_guidelines.pdf --auto-fix --provider mock
LLM_PROVIDER=ollama-small python src/cli.py build data/brands/nordlys-energy_guidelines.md
```

Providers work the same as in my other repos: `ollama`, `ollama-small` (8 GB laptops), `gemini`, `openai`, `anthropic`, `mock`. Auto-detected if not set. Models per role are in [`config/models.json`](config/models.json).
...
- [Case study](docs/CASE_STUDY.md): the real story, my role, and what I'd change
